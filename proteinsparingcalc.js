...
function startUpTheCalculator() {
    ...
    function setUpForSelectedGender(){
        ...
        if (estimateBodyFatLevelDescriptions != null) {
    ...
    function getUserBodyFatPercentage(){
        if (user != null) {
            user.bodyFatPercentage = bodyFatPercentageInputBox.value;
        }
    }
    ...
    function calcProteinFastPlan(){
        console.log('calculating protein fast plan');
    ...
    function calculateProteinAmount(){
        console.log('calculating protein amount');

        if (user.gender == 'Male') {
            if (user.bodyFatPercentage > .26) {
                recommnededProteinAmount = 1
            }
            if (user.bodyFatPercentage > .15) {
                recommnededProteinAmount = 1.5
            }
        } else {
            if (user.bodyFatPercentage > .36) {
                recommnededProteinAmount = 1
            }
            if (user.bodyFatPercentage > .25) {
                recommnededProteinAmount = 1.5
            }
        }

        user.dailyProteinIntakeRecommended = user.leanBodyMass * recommnededProteinAmount;
    }

    const processProteinFastPlan = debounce(() => calcProteinFastPlan());

    const bodyFatPercentageInputBox = document.getElementById('bodyFatPercentageInputBox');
    if (bodyFatPercentageInputBox != null) {
        bodyFatPercentageInputBox.addEventListener('keyup', processProteinFastPlan);
    };

    const weightInputBox = document.getElementById('weightInputBox');
    if (weightInputBox != null) {
        weightInputBox.addEventListener('keyup', processProteinFastPlan);
    }
